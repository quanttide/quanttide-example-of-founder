...
4. **规则分两个维度归类**：
   - **母题（写什么）**：反复出现的主题、人物原型、情感内核、设定倾向——"我总在写什么"（如：理性与感性自我的相遇、"把不敢说的变成可以说的"、创业者滤镜的破碎）。母题是作者关怀的显影，跨作品重复出现。
   - **风格（怎么写）**：反复出现的操作手法——"我习惯怎么写"（如：涌现而非设计、物证化、独白→问答、第一人称行动宣言收尾）。风格是可执行的手艺规则。
   - 每条规则先判断属于哪个维度再入类；判不定归属的不硬塞。
5. 证据链强制：每条规则必须能指回日志原文，指不回的不要写。宁可少，不可编。
6. 保守输出：素材不足以支撑任何规则时，如实说"素材不足以提炼规则"，并指出素材属于哪类（如心境记录、完结告别）为何不适合立规则。不要硬凑条目。
7. 粒度对齐：一条规则 = 一个可执行的操作原则 + 例证。过泛（如"要多改几遍"）或只适用一次的偶发操作都不写。
8. 冲突保留：素材中互相矛盾的实践如实记为"作者权衡项"，不强行统一。
...
最后附一段"本质归纳"：这些母题与风格共同指向的交换——删掉/留下的是什么。
