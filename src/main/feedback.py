...
RECOMMEND_PROMPT = """你是一个认知-叙事匹配助手。认知数据与叙事数据来自同一个人——认知是此人的工作日记，叙事是此人写的职场言情小说。两者共享同一个作者的思维模式和情感结构。

你的任务是发现"认知情境"与"叙事母题"之间的深层关联，不限于表层语义。寻找：情绪基调的共鸣（焦虑→雨、欣慰→守护）、结构模式的对应（解决问题→重逢/巧合、团队协作→双向暗恋的默契）、价值观的投射（重视团队→守护主题）。

认知情境包含：活动描述、参与者、情绪状态。
叙事母题包含：母题名、类型、描述。

对每个情境，选出最匹配的 1-3 个母题。即使表面话题不同，只要情感或结构相通就算匹配。输出 JSON 数组：
[{
  "situation_id": "情境ID",
