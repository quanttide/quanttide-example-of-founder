...
fn extract(text: &str) -> serde_json::Value {
    if text.is_empty() { return serde_json::json!({}); }
    let llm = LLM::default();
    match llm.complete(&[Message::new("system", EXTRACT_PROMPT), Message::new("user", &text[..text.len().min(4000)])], CompleteOptions::default()) {
...
fn cmd_check(memory: &str, fiction: &str, days: u32) {
    let (mem, fic) = (git_latest(&expand(memory), days, 100), git_latest(&expand(fiction), days, 100));
    println!("日记: {}字  小说: {}字", mem.len(), fic.len());
    let (d, f) = (extract(&mem), extract(&fic));
    let (dv, fv) = (d["valence"].as_f64().unwrap_or(0.0), f["valence"].as_f64().unwrap_or(0.0));
    println!("现实: {} ({})  创作: {} ({})  差距: {:.1}", d["dominant_mood"].as_str().unwrap_or("?"), dv, f["dominant_mood"].as_str().unwrap_or("?"), fv, fv - dv);
}
...
fn cmd_track(memory: &str, fiction: &str, csv_path: Option<String>, show: bool) {
    ...
    if show { show_trend(&csv_path); return; }
    let (mem, fic) = (git_latest(&expand(memory), 7, 100), git_latest(&expand(fiction), 7, 100));
    let (d, f) = (extract(&mem), extract(&fic));
    let (dv, fv) = (d["valence"].as_f64(), f["valence"].as_f64());
    let gap = match (dv, fv) { (Some(d), Some(f)) => format!("{:.1}", f - d), _ => String::new() };
    let now = chrono_now();
    let week = format!("{}-W{:02}", now.0, now.1);
    let mut wtr = csv::WriterBuilder::new().from_path(&csv_path).unwrap();
    wtr.write_record(&["week","date","diary_present","diary_mood","diary_valence","fiction_present","fiction_mood","fiction_valence","gap","signals"]).ok();
    wtr.write_record(&[week.as_str(), &format!("{}-{:02}-{:02}", now.0, now.1, now.2), if mem.is_empty(){"0"}else{"1"}, d["dominant_mood"].as_str().unwrap_or(""), &dv.map(|v|format!("{v}")).unwrap_or_default(), if fic.is_empty(){"0"}else{"1"}, f["dominant_mood"].as_str().unwrap_or(""), &fv.map(|v|format!("{v}")).unwrap_or_default(), &gap, &f["warning_signs"].as_array().map(|a| a.iter().map(|v| v.as_str().unwrap_or("")).collect::<Vec<_>>().join("; ")).unwrap_or_default()]).ok();
    wtr.flush().ok();
    println!("已记录 {week} → {csv_path}");
}

fn show_trend(csv_path: &str) {
    let mut rdr = csv::Reader::from_path(csv_path).unwrap_or_else(|_| { println!("暂无数据"); std::process::exit(0); });
    let rows: Vec<_> = rdr.records().filter_map(|r| r.ok()).collect();
    println!("共 {} 周\n", rows.len());
    for r in rows.iter().rev().take(10).rev() { println!("{:<10} {:<12} 日记:{:<6} 小说:{:<6} 差距:{:<6}", r.get(0).unwrap_or(""), r.get(1).unwrap_or(""), r.get(4).unwrap_or(""), r.get(7).unwrap_or(""), r.get(8).unwrap_or("")); }
...
fn cmd_profile(memory: &str, fiction: &str, output: &str) {
    let (mem, fic) = (daily_content(&expand(memory), 500), daily_content(&expand(fiction), 500));
    let (mem_sample, fic_sample): (Vec<_>, Vec<_>) = (mem.iter().take(20).collect(), fic.iter().take(20).collect());
    println!("分析 {} 天日记 + {} 天小说...", mem_sample.len(), fic_sample.len());
    let (mut d_vals, mut f_vals) = (Vec::new(), Vec::new());
    for (_, text) in &mem_sample { if let Some(v) = extract(text)["valence"].as_f64() { d_vals.push(v); } }
    for (_, text) in &fic_sample { if let Some(v) = extract(text)["valence"].as_f64() { f_vals.push(v); } }
    let (da, ds, fa) = (mean(&d_vals), stddev(&d_vals, mean(&d_vals)), mean(&f_vals));
...
fn chrono_timestamp_to_str(secs: i64) -> String { let t = std::time::UNIX_EPOCH + std::time::Duration::from_secs(secs as u64); let dt = time_to_date(t); format!("{}-{:02}-{:02}", dt.0, dt.1, dt.2) }
fn chrono_now() -> (i32, u32, u32) { time_to_date(std::time::SystemTime::now()) }
fn time_to_date(t: std::time::SystemTime) -> (i32, u32, u32) {
    let secs = t.duration_since(std::time::UNIX_EPOCH).unwrap().as_secs();
    let days = secs / 86400; let year = 1970 + (days as f64 / 365.25) as u64;
    let rem = days as i64 - ((year - 1970) as i64 * 365 + leap_days(1970, year));
    let month = [31,28,31,30,31,30,31,31,30,31,30,31]; let mut d = rem; let mut m = 0;
    for (i, days_in_m) in month.iter().enumerate() { if d < *days_in_m as i64 { m = i + 1; break; } d -= *days_in_m as i64; }
    if m == 0 { m = 12; d = 31; } (year as i32, m as u32, d as u32 + 1)
}
fn leap_days(start: u64, end: u64) -> i64 { ((end - start) / 4) as i64 }
fn daily_content(repo: &PathBuf, max: usize) -> Vec<(String, String)> {
    ...
    let _ = revwalk.push_head(); revwalk.set_sorting(git2::Sort::TIME).unwrap_or(());
    ...
    for oid in revwalk.flatten().take(max) {
        ...
        for entry in tree.iter().take(3) {
            if let Some(obj) = entry.to_object(&repo).ok() { if let Some(blob) = obj.as_blob() { let content = std::str::from_utf8(blob.content()).unwrap_or(""); days.entry(date.clone()).or_default().push(content[..content.len().min(2000)].to_string()); break; } }
        }
    }
    let mut result: Vec<_> = days.into_iter().collect(); result.sort_by(|a, b| a.0.cmp(&b.0));
    result.iter().map(|(d, texts)| (d.clone(), texts.join("\n\n"))).collect()
}
fn mean(v: &[f64]) -> f64 { if v.is_empty() { 0.0 } else { v.iter().sum::<f64>() / v.len() as f64 } }
fn stddev(v: &[f64], m: f64) -> f64 { if v.len() < 2 { 0.0 } else { (v.iter().map(|x| (x - m).powi(2)).sum::<f64>() / v.len() as f64).sqrt() } }
