...
class ReviewEngine:
            ...
            if blank_run >= 2 and not self._is_continuation(stripped):
                scenes.append(
                    (
                        current_start,
                        i - blank_run - 1
                        if i - blank_run - 1 >= current_start
                        else current_start,
                    )
                )
                current_start = i
        ...
        for i, line in enumerate(self.lines, 1):
            ...
            if re.search(r'[「「『""]', stripped):
                ...
                if (
                    ...
                    and not self._has_action(stripped)
                ):
                    self.gaps.append(
                        Gap(
                            "对话间隙",
    ...
    def _has_action(self, line: str) -> bool:
        return self._is_action_line(line) or bool(
            re.search(r"沉默|停顿|低头|抬头|深呼吸", line)
        )
    ...
    def _analyze_style(self):
        ...
        self.styles.append(
            StyleCheck(
                "对话 → 动作",
                min(100, int(action_ratio * w["action_conversion_multiplier"])),
        ...
        identity_score = (
            max(0, 100 - identity_matches * w["identity_label_penalty"])
        ...
        hidden_score = max(
            0, 100 - (emotion_matches + named_emotions) * w["emotion_label_penalty"]
        )
        self.styles.append(
            StyleCheck(
                "命名情绪 → 隐藏情绪",
        ...
        if not self.dual_pov:
            ...
            perspective_score = (
                max(0, 100 - (overage / expected_shifts) * 100) if overage > 0 else 100
            )
            self.styles.append(
                StyleCheck(
                    "有限第三人称紧贴",
        ...
        for line in last_lines:
            if any(m in line for m in plot_markers):
                ...
                break
        ending_score = (
            100 if self.stats["state_endings"] > self.stats["plot_endings"] else 30
        )
        self.styles.append(
            StyleCheck(
                "状态句结尾",
    ...
    def print_report(self):
        ...
        print(
            f"  • 场景长度: 最短{min(scene_lens)}行 / 最长{max(scene_lens)}行 / 平均{sum(scene_lens) // len(scene_lens)}行"
        )
        if self.stats["scene_count"] >= 2:
            print(
                f"  • 进场景方式: 以动作开始 {self._count_scene_starts('action')} 个 / 以叙述开始 {self._count_scene_starts('narrative')} 个"
            )
...
class ThreeRSession:
    ...
    def run(self):
                ...
                self.log.append(
                    {
                        ...
                        "delta_chars": delta,
                        "score": int(
                            sum(s.score for s in engine.styles)
                            / max(len(engine.styles), 1)
                        ),
                    }
                )
        ...
        for entry in self.log:
            print(
                f"  第{entry['cycle']}轮 | 修改{entry['delta_chars']:+d}字 | 风格评分:{entry['score']}"
            )
        print(f"{'=' * 60}\n")
...
def main():
    ...
    p_review.add_argument(
        "--dual-pov", action="store_true", help="声明式豁免：交替双视角不扣分"
    )
